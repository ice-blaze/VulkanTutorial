...
private:
  ...
  void createCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
      ...
      // vkCmdDraw(commandBuffers[i], static_cast<uint32_t>(vertices.size()), 1, 0, 0); // before index buffer
  ...
  void createGraphicsPipeline() {
    ...
    rasterizer.lineWidth = 1.0f; // maybe no need if we are in fill mod ?
    rasterizer.cullMode = VK_CULL_MODE_BACK_BIT;
    // rasterizer.frontFace = VK_FRONT_FACE_CLOCKWISE; // because of the Y axis inverted
